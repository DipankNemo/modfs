...
fifth is regenerated by its own tool, which the caller runs afterwards:
  ...
  /var/lib/apt/extended_states    manual wins over automatic      (here)
...
def merge_extended_states(layers, merged):
    # APT records automatic installations positively; manual is usually an
    # ABSENT stanza. Unioning only those positive records loses a sibling's
    # explicit request. Infer manual packages from each complete dpkg status,
    # and let manual in ANY layer win over automatic in another.
    # Siblings all inherit the first (base) layer. An absent extended_states
    # file in a delta therefore inherits base's flags, not an empty database.
    rel = 'var/lib/apt/extended_states'
    base_text = read(os.path.join(layers[0][1], rel)) if layers else None
    keep, order, manual = {}, [], set()
    for name, root in layers:
        text = read(os.path.join(root, rel))
        if text is None:
            text = base_text
        automatic = set()
        for s in stanzas(text):
            key = (field(s, 'Package'), field(s, 'Architecture'))
            if key[0] is None: continue
            if field(s, 'Auto-Installed') == '1':
                automatic.add(key[0])
            if key not in keep: order.append(key)
            keep[key] = s
        installed = {field(s, 'Package') for s in
                     stanzas(read(os.path.join(root, 'var/lib/dpkg/status')))
                     if (field(s, 'Status') or '').split()[-1:] == ['installed']}
        # Single-architecture scope: APT records arch=amd64 even for packages
        # whose dpkg Architecture is 'all'. Match flags by package NAME while
        # retaining APT's original architecture in emitted records.
        manual.update(installed - automatic - {None})
    selected = [keep[k] for k in order if k[0] not in manual
                and field(keep[k], 'Auto-Installed') == '1']
    # Always write, including zero records, to clear a higher layer's stale
    # automatic flags when every surviving package is manually requested.
    write(os.path.join(merged, rel), '\n\n'.join(selected) + ('\n' if selected else ''))
    return len(selected)
